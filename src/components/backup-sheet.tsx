...
export function BackupSheet({ visible, state, onClose, onRestore, onWipe, onToast }: { visible: boolean; state: AppState; onClose: () => void; onRestore: (state: AppState) => Promise<boolean>; onWipe: () => Promise<boolean>; onToast: (message: string) => void }) {
  ...
  return <BottomSheet onClose={onClose} title="Daten sichern" visible={visible}>
    ...
    <View style={styles.actions}><Button onPress={saveFile}>Als Datei sichern</Button><Button kind="line" onPress={async () => { await Clipboard.setStringAsync(text); onToast('Kopiert'); }}>Text kopieren</Button><Button disabled={restoring} kind="line" onPress={restore}>Aus Text wiederherstellen</Button><Button kind="danger" onPress={() => Alert.alert('Alle Daten löschen?', 'Dieser Schritt kann nicht rückgängig gemacht werden.', [{ text: 'Abbrechen' }, { text: 'Alles löschen', style: 'destructive', onPress: () => { const remove = async () => { if (await onWipe()) onClose(); }; void remove(); } }])}>Alle Daten löschen</Button></View>
  </BottomSheet>;
}
